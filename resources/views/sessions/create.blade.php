...
@section('content')
    <div class="max-w-2xl px-4 py-8 mx-auto">
        <h2 class="mb-6 text-5xl font-semibold text-center text-indigo-500">{{ __('keywords.create_session') }}</h2>

        <form action="{{ route('session.store') }}" method="POST" class="p-6 space-y-6 bg-white shadow-md rounded-xl">
            @csrf

            <div>
                <label for="start_time" class="block mb-1 text-sm font-medium text-gray-700">{{ __('keywords.start_time') }}</label>
